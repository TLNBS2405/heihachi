...
CONFIG_PATH = configurator.Configurator(
    os.path.abspath(os.path.join(base_path, "resources", "config.json"))
)
CHARACTER_LIST_PATH = os.path.abspath(
    os.path.join(base_path, "resources", "character_list.json")
)
    ...
    if character_name:
        ...
        if move_type:
            ...
            if character_move:
                ...
                similar_moves_embed = embed.similar_moves_embed(
...
async def on_message(message):
    if (
        not util.is_user_blacklisted(message.author.id)
        and message.content
        and message.author.id != hei.user.id
    ):
...
async def self(interaction: discord.Interaction, character_name: str, move: str):
    if not (
        util.is_user_blacklisted(interaction.user.id)
        or util.is_author_newly_created(interaction)
    ):
...
async def self(interaction: discord.Interaction, message: str):
    if not (
        ...
        try:
            ...
            await channel.send(
                content=feedback_message, view=button.DoneButton(actioned_channel)
            )
